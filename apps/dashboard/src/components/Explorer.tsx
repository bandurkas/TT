...
export const ProductPill = ({ s }: { s: string }) => {
  ...
  return <Pill tone={m.tone}>{t(m.label)}</Pill>;
};
export const VideoPill = ({ c }: { c: VideoClass | null | undefined }) => {
  const t = useT();
  const m = c ? VCLASS[c] : null;
  return m ? <Pill tone={m.tone}>{t(m.label)}</Pill> : <Pill tone="gray">—</Pill>;
};

const neg = (v: string | null) => idr(-(num(v) ?? 0));
...
export default function Explorer({ tab, setTab, products, videos, campaigns, creators }: Props) {
  ...
  return (
    <section className="zone">
      ...
      <div className="card">
        <div className="tabs" role="tablist">{TABS.map(([k, l]) => <button key={k} role="tab" aria-selected={cur === k} className={cur === k ? "on" : ""} onClick={() => setTab(k)}>{t(l)}</button>)}</div>
        {L.error && <div style={{ padding: 12 }}><ErrorNote error={L.error} onRetry={L.reload} /></div>}
                  ...
                  {products.data.rows.map((r) => {
                    ...
                    return (
                      <tr key={r.product_id} title={r.status_reason}>
                        ...
                        <td className="r">{neg(r.fees)}</td><td className="r">{neg(r.cogs)}</td>
                        <td className="r" title={products.data?.ad_cost_note}>{neg(r.ad_cost)} <span className="tiny">{t("est.")}</span></td>
                ...
                <div className="gallery">
                  {videos.data.cards.map((v) => (
                    <div className="vcard" key={v.video_id}>
                      ...
                      <div className="vb">
                        <span className="id">Video {shortId(v.external_video_id ?? v.video_id)}</span>
                        ...
                        <span className="k">{t("Clicks")} <span className="tiny">({t("derived (views × CTR)")})</span></span><span className="v">{int(v.clicks, lang)}</span>
                        ...
                        <span className="rs">{t("Confidence")} {t(v.confidence)} · {v.reasons.join("; ")}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="scroll"><table className="tbl">
                ...
                <tbody>
                  {creators.data.rows.map((r, i) => (
                    <tr key={i}><td>{t(r.creator)}</td><td className="r">{int(r.orders, lang)}</td><td className="r">{idr(r.gmv, lang)}</td><td className="r dn">{r.affiliate_commission == null ? "—" : neg(r.affiliate_commission)}</td><td className="r">{idr(r.profit_after_commission, lang)}</td></tr>
                  ))}
                </tbody>
