...
import type { CreativeBridge } from "@/lib/types";
import { Pill } from "./ui";

const TONE = { scale: "good", hold: "warn", cut: "bad", no_orders: "bad", no_data: "gray" } as const;

function verdictLabel(v: string, ru: boolean) {
  ...
  return (
    <div className="scroll" id="panel-bridge" role="tabpanel" aria-labelledby="tab-bridge">
          ...
          {data.products.map((p) => {
            ...
            return (
              <tr key={p.product_id}>
                ...
                <td><Pill tone={TONE[p.verdict as keyof typeof TONE] ?? "gray"}>{verdictLabel(p.verdict, ru)}</Pill></td>
              </tr>
            );
          })}
        </tbody>
      ...
      <table className="tbl">
        ...
        <tbody>
          {!videos.length && <tr><td colSpan={6} className="empty">{ru ? "Ни одно видео не набрало просмотров за период." : "No video gathered views in this period."}</td></tr>}
          {videos.map((v) => (
            <tr key={v.video_id}>
              <td>{shortId(v.external_video_id ?? String(v.video_id))}</td>
              ...
              <td style={{ whiteSpace: "normal", minWidth: 200 }}>
                {v.products.length === 0 ? <span className="muted">—</span> : v.products.map((pid) => {
                  ...
                  if (!p) return null;
                  return <Pill key={pid} tone={TONE[p.verdict as keyof typeof TONE] ?? "gray"}>{(p.title ?? "").slice(0, 26)}</Pill>;
                })}
              </td>
            </tr>
          ))}
      ...
      <div className="note dn" style={{ margin: 12 }}>
        ...
        <span>{ru ? "GMV Max относит 96% заказов в неатрибутированную корзину, поэтому рекламный расход на конкретное видео не существует как измеримая величина. Видео судится по GPM — сколько денег приносит тысяча просмотров — и по тому, какие товары оно несёт: цветом отмечено, зарабатывает ли товар свою рекламу."
                 : "GMV Max leaves 96% of orders in an unattributed bucket, so ad spend per video does not exist as a measurable quantity. A video is judged on GPM — money per thousand views — and on the products it carries, coloured by whether each earns its advertising."}</span>
      </div>
    </div>
  );
}
