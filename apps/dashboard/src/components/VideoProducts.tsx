    ...
    <section className="zone">
      ...
      {loading && !vp ? <Skeleton h={260} /> : vp && sp && (
        <>
                  ...
                  return (
                    <div key={l.lag_days} className="card" style={{ padding: "8px 10px", borderColor: best ? "var(--accent)" : undefined }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                        <span className="small"><b>{t("Video views")}</b> → {t("Product card GMV")} {lagLabel(l.lag_days, t)}</span>
          ...
          <div className="note">{vp.notes.join(" · ")} · {t("correlation ≠ causation")}<EnHint lang={lang} /></div>
        </>
      )}
    </section>
  );
