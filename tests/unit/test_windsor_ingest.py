...
def test_client_requests_a_gmv_max_field_and_never_leaks_the_key():
    urls = []
    rows, meta = client({"data": [{"date": "2026-08-26", "campaign_id": "1", "gmv_max_ads_spend": 5}]},
                        urls).fetch_gmv_max(date(2026, 8, 26), date(2026, 8, 26))
    ...
    assert "api_key=k" in urls[0] and "api_key=k" not in meta["url"] and meta["url"].endswith("***")
...
def test_ingest_writes_only_the_days_the_connector_reported(monkeypatch):
    ...
    monkeypatch.setattr(W, "_stored_cost", lambda *a: None)
...
def test_ingest_stores_raw_and_writes_nothing_when_there_are_no_rows():
    ...
    assert out == {"days": 0, "written": 0, "campaigns": 0, "disagreements": [], "note": "no rows"}
...
def test_ingest_reports_a_material_disagreement_instead_of_hiding_it(monkeypatch):
    ...
    monkeypatch.setattr(W, "_stored_cost", lambda *a: W.number("73989"))
    ...
    monkeypatch.setattr(W, "_stored_cost", lambda *a: W.number("339256"))   # agreement is silent
...
def test_ingest_keeps_the_open_day_partial_and_skips_a_newer_observation(monkeypatch):
    ...
    assert out["written"] == 1 and "newer" in out["disagreements"][0]["skipped"]
