...
- **A null is not a zero.** The connector answers `null` for any field it cannot fill. A null
  ...
  day**. Only a day that has **ended** can be missing anything: nothing on file, or a figure
  still marked `partial` — a mid-day reading, since the manual form defaults `final` to false — means
  the day is understated and would otherwise sit under a green job. Nothing is owed for the day still
  running, with or without a row. Reasoning: with
  ...
  rejections first (the day's Cost never landed), then an unusable `account_id` (campaign attribution
  is lost, but no day loses its Cost), then null days. The account and null-day entries are computed before the day loop, because nothing in it can change
  their state; rejections necessarily accumulate during it. This is not crash-proofing: an exception
  other than `ValueError` propagates out of `ingest`, and `_run_job` records that instead — visible in
  `/health`, but without any of this detail. Only
